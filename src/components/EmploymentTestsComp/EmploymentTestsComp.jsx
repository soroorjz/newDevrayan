...
const EmploymentTestsComp = ({ exams, statusTitle, title }) => {
  const { user } = useAuth();

  // فیلتر کردن آزمون‌ها بر اساس statusTitle
  const getFilteredExams = () => {
    if (!exams || exams.length === 0) {
      console.log(`No exams data for ${statusTitle}`);
      return [];
    }

    // اصلاح عناوین برای تطبیق با JSON
    const statusMap = {
      "درحال ثبت نام": "در حال ثبت‌نام",
      "آزمون کتبی برگزار شده": "برگزار شده",
      "در انتظار": "در حال آماده‌سازی",
      "دریافت کارت ورود به جلسه": "در انتظار صدور کارت",
    };

    const mappedStatus = statusMap[statusTitle] || statusTitle;

    const filtered = exams.filter((exam) => exam.examStatus === mappedStatus);
    console.log(`Filtered exams for ${statusTitle}:`, filtered);
    return filtered;
  };
  ...
  // اگر داده‌ها وجود نداشته باشن (برای شبیه‌سازی لودینگ)
  if (!exams) {
    console.log(`Still loading exams for ${statusTitle}...`);
    return (
      <div className="active-exams-sj">
  ...
  return (
    <div className="active-exams-sj">
      ...
      {examData && examData.length > 0 ? (
        ...
        >
          {examData.map((exam) => (
            <SwiperSlide key={exam.examId} className="exam-card-sj">
              <div className="exam-title-sj">{exam.examName}</div>
              <div className="exam-deadline-sj">
                تاریخ برگزاری آزمون: {exam.eventDate}
              </div>
              <div className="examCard-Footer">
                {!user && (
                  <Link to="/signUpForm" key="signup">
