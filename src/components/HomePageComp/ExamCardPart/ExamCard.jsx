...
import { useAuth } from "../../../AuthContext";
import { Link } from "react-router-dom";
import examsData from "../../../exams.json"; // مسیر فایل JSON
import "./ExamCard.scss";
import CardListSkeleton from "./CardListSkeleton";
...
const ExamCard = () => {
  ...
  // شبیه‌سازی prefetch برای جزئیات آزمون (فقط برای حفظ رفتار قبلی)
  const prefetchExamInfo = (examId) => {
    console.log("Prefetching exam info for ID:", examId);
    const exam = examsData.find((exam) => exam.examId === examId);
    console.log("Prefetched exam:", exam);
  };

  // داده‌ها مستقیماً از JSON می‌آن
  const exams = examsData || [];

  // اگر داده‌ای وجود نداشته باشه (اختیاری برای شبیه‌سازی لودینگ)
  if (!exams.length) {
          ...
          {exams.map((examCard) => (
            <SwiperSlide key={examCard.examId}>
              ...
              >
                ...
                <div className="examCard-details">
                  <p className="examCard-Status detail">
                    وضعیت آزمون: <span>{examCard.examStatus}</span>
                  </p>
