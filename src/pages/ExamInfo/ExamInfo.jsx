...
const ExamInfo = () => {
  ...
  // پیدا کردن آزمون بر اساس examId
  const exam = examsData.find((exam) => exam.examId === parseInt(id));
  ...
  useEffect(() => {
    ...
    if (exam && window.innerWidth > 728) {
      console.log("Starting tutorial with exam:", exam);
      const hasSeenTutorial = localStorage.getItem("hasSeenExamTutorial");
      if (!hasSeenTutorial) {
        setTimeout(() => {
          console.log("Attempting to find #RegistrationBtn");
  ...
  return (
    <div className="examInfoContainer">
      ...
      />
      <Countdown
        registrationDeadline={exam.endDate}
        startDate={exam.startDate}
      />
      <ExamInfoCard
        startDate={exam.startDate}
        endDate={exam.endDate}
        cardIssueDate={exam.cardIssueDate}
